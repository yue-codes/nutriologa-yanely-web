...
- **Astro 7**, with **Preact** wired in (`@astrojs/preact`) for interactive components, and **Tailwind CSS v4** via the Vite plugin in `astro.config.mjs` (CSS-first config, no `tailwind.config.js`).
...
- `Navbar.astro` is a floating "glass pill": the `<header>` itself is just a transparent `sticky` wrapper (`pt-4`), and the visible rounded-full bar (blur + tint) is the inner `<div>`. That makes the header's total height `5rem` (`pt-4` + `h-16`), not `4rem` — `MobileNav.tsx`'s full-screen overlay (`top-20`) and `Hero.astro`'s `min-h-[calc(100dvh-5rem)]` both depend on that figure; if the pill's padding or height ever changes, update those two together.
...
- `src/data/servicios.ts`, `src/data/testimonios.ts`, `src/data/contacto.ts`, `src/data/libros.ts` are the single source of truth for those lists — pages and components read from them, never hardcode service/testimonial/contact/book text inline.
- Each entry in `categoriasServicios` (`servicios.ts`) also carries `imagen`/`imagenAlt`, pointing to a photo in `public/servicios/`; `ServiceCategoryCard.astro` renders it as the card's header image (same image-on-top pattern as `BlogCard.astro`).
- The blog is an Astro Content Collection: schema in `src/content.config.ts` (the Content Layer API — `glob()` loader from `astro/loaders`, not the legacy `src/content/config.ts`), entries as Markdown files in `src/content/blog/`. `entry.id` (used for the `/blog/[id]` route) is the filename slug, derived automatically by the loader. Two fields, `ctaTexto`/`ctaWhatsappMensaje`, are optional and only set on posts that should end with a WhatsApp button (`blog/[id].astro` renders it conditionally) — most posts omit them.
- `libros.ts` holds an array (`Libro[]`) so it can grow to multiple books later, but `home/LibroPreview.astro` currently only renders `libros[0]` — there's no listing/index page for books yet, just the one featured on Inicio. Each book's `blogSlug` points at the `src/content/blog/` entry that carries its actual sales pitch and WhatsApp CTA (via the two optional fields above); the home card itself only links there with a "Más información" button, it has no CTA of its own.
...
File-based routing in `src/pages/`: `index.astro`, `sobre-mi.astro`, `servicios.astro`, `blog/index.astro`, `blog/[id].astro`, `contacto.astro`. Inner pages share `ui/PageHeader.astro` for their banner; `index.astro` is a thin composition of section components from `src/components/home/` (`Hero.astro`, `ServiciosPreview.astro`, `TestimoniosPreview.astro`, `LibroPreview.astro`, `BlogPreview.astro`, `CtaFinal.astro`) — each fetches its own data and is single-use, which is why they live apart from the reusable atoms in `ui/`/`cards/`/`layout/`. Most of those sections share `ui/Section.astro`, a generic wrapper that handles the alternating tinted/plain background rhythm and an optional `SectionHeading`; `Hero.astro` doesn't fit that pattern and renders its own `<section>`, and `LibroPreview.astro` uses a bare `<Section>` (no `eyebrow`/`titulo`) because its own heading ("Presento mi libro: ...") already does that job — keep that section untinted, since it sits between the untinted `TestimoniosPreview` and the tinted `BlogPreview` in the tinted/plain alternation.
